...
\begin{question}
  \begin{enumerate}[label= (\alph*)]
    ...
    \item
          ...
          In a more general sense, the $\Diamond^{\leq k}\psi$ operator could be incorporated into LTL model checking by constructing it's negation as a series of $k$ linear states followed by a terminating state in which if $\psi$ was true the NFA returned to the initial state and if $\psi$ was not true it would continue to the next state until it reached the final, accepting, position.
